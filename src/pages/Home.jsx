...
/**
 * Month calendar with month + year pickers. Days that have announcements get a rust dot and are
 * clickable; the rest are dim. Arrows step months; the selects jump anywhere; "Jump to latest"
 * returns to the most recent announcement day.
 */
function Calendar({ view, setView, datesWith, selected, onSelect, years = [], latest, onJumpLatest }) {
  const { y, m } = view
    ...
    <div className="rounded-lg border border-rule p-4 sm:p-5">
        ...
        {cells.map((d, i) => {
          ...
          return (
            <button key={i} type="button" disabled={!has} onClick={() => onSelect(key)}
              aria-label={has ? `${MONTHS[m]} ${d}, view announcements` : undefined}
      ...
      {latest && selected !== latest && (
        <button type="button" onClick={onJumpLatest}
          ...
          &larr; Jump to latest ({new Date(latest + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric' })})
        </button>
      )}
    </div>
  )
